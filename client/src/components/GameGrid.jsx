...
const GameGrid = memo(({ data }) => {
  const { games = [], images = [] } = data;

  const getImageForGame = (universeId) => {
    const imageEntry = images.find((img) => img.id === universeId);
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto w-11/12 sm:w-5/6 lg:w-4/5">
        <h2 className="mb-8 text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight"></h2>
